...
import { AbilityContainer } from '../Entity/index.js';
import { ResourceAbilityContainerSpawnSystemConfig } from '../Resource/ResourceAbilityContainerSpawnSystemConfig.js';
import { Vector } from '../../AlkkagiShared/Modules/Vector.js';
import { Random } from '../../AlkkagiShared/Modules/Random.js';

class AbilityContainerSpawnSystem extends System {
    constructor(world) {
        super(world);

        this.config = ResourceAbilityContainerSpawnSystemConfig.get(0);
        this.occupiedSlots = new Set();
    }

    getSystemID() {
        return 'AbilityContainerSpawnSystem';
    }
    ...
    onStart() {
        ...
        for(let i = 0; i < this.config.spawnCount; i++) {
            this.spawnAbilityContainer();
        }
    }

    spawnAbilityContainer() {
        // 비어있는 슬롯 중에서 선택
        ...
        const abilityContainer = new AbilityContainer(this.world, onContainerGone.bind(this));
        abilityContainer.position = this.getRandomPosition(slotIndex);
        this.world.addEntity(abilityContainer);
    }
...
export { AbilityContainerSpawnSystem };